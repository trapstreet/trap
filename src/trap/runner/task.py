...
class TaskRunner:
    ...
    def run(
        ...
        *,
        ...
        Raises ConfigError, before any case starts, for a task that could hand a
        solution the answers (see ``refuse_answer_leaks``)."""

        cases = tuple(cases)
        refuse_answer_leaks(self.traptask_dir, self.traptask_config, cases)
        case_results = tuple(
            self._iter_cases(
                cases,
                fail_fast=fail_fast,
