...
- **`dirs.inputs` / `dirs.expected`** — case input / expected dirs, relative to `traptask.yaml` (defaults `inputs/`, `expected/`). trap refuses to run a task that could hand a solution the answers. Nothing a solution is handed may be a symlink: not the directory of a case about to run, not a directory between the `inputs` dir and it, and nothing inside it (the `inputs` dir itself may be a link). No answers directory — the `expected` dir, or any case's `expected/<id>`, skipped or not selected included — may lie inside a running case's inputs, and no case's `expected/<id>` may be or lie around a running case's directory; directories are compared by identity on disk, so another letter case or Unicode spelling of one directory counts. Case ids stay inside their directories: no absolute id, no `..`. An answer placed in the inputs — a copy, a hard link, or a link in `expected/` to an input file — isn't checked, and is the task author's to avoid.
