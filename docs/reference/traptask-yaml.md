...
- **`dirs.inputs` / `dirs.expected`** — case input / expected dirs, relative to `traptask.yaml` (defaults `inputs/`, `expected/`). trap refuses to run a task that could hand a solution the answers. No directory on the way to a case about to run may be a symlink: not the case's directory, not a directory between the `inputs` dir and it (the `inputs` dir itself may be a link). Inside the case's directory a symlink is allowed only when it resolves to a regular file under the `inputs` dir and outside every answers directory — a file shared by many cases, kept in a folder that is not a case (`inputs/context/`, say); a link that leaves the `inputs` dir, reaches the answers, points at a directory, dangles or loops is refused. No answers directory — the `expected` dir, or any case's `expected/<id>`, skipped or not selected included — may lie inside a running case's inputs, and no case's `expected/<id>` may be or lie around a running case's directory; directories are compared by identity on disk, so another letter case or Unicode spelling of one directory counts. Case ids stay inside their directories: no absolute id, no `..`. An answer placed in the inputs — a copy, a hard link, or a link in `expected/` to an input file — isn't checked, and is the task author's to avoid.
