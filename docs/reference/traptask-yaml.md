...
- **`dirs.inputs` / `dirs.expected`** — case input / expected dirs, relative to `traptask.yaml` (defaults `inputs/`, `expected/`). trap refuses to run a task where a case's inputs are, contain, or sit inside its own expected answers, contain the `expected` dir, or sit inside it outside the `inputs` dir — `inputs/<id>` linked into `expected/`, or both dirs set to one directory. (An `expected` dir around `inputs`, such as `./`, is fine.) The check follows links on each case directory's own path and on both dirs and compares the real directories, so a different letter case or Unicode spelling of one directory on macOS counts; it doesn't look at links inside a case directory (the built-in shapes refuse those), and a hard link or a copy of an answer is the author's to avoid.
