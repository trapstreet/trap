...
- **`dirs.inputs` / `dirs.expected`** — case input / expected dirs, relative to `traptask.yaml` (defaults `inputs/`, `expected/`). trap refuses to run a task where the inputs of a case about to run are, contain, or sit inside any case's answers (`expected/<id>` — its own, or that of any case the task defines, skipped or not selected included), contain the `expected` dir, or sit inside it — e.g. `inputs/<id>` linked into `expected/`, `expected/<id>` linked to another case's inputs, both dirs set to one directory, or case ids that put one case's answers at another's inputs (`c2` and `inputs/c2` with `expected: ./`). The one exception: when the `expected` dir is around the `inputs` dir (such as `./`), a case directory under `inputs` doesn't count as inside the `expected` dir — it still must not be, contain, or sit inside any case's answers. The check follows links on each case directory's own path and on both dirs and compares the real directories, so a different letter case or Unicode spelling of one directory on macOS counts; it doesn't look at links inside a case directory (the built-in shapes refuse those), and a hard link or a copy of an answer is the author's to avoid.
